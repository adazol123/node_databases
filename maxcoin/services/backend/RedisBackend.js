...
class RedisBackend {
  ...
  async max() {
    console.info('Connection to RedisDB');
    console.time('redis-connect');
    const client = this.connect();
    if (client) {
      console.info('Sucessfully connected to Redis')
    } else {
    ...
    console.info(`Inserted ${insertResult} document into Redis`)

    console.info('Queryinh Redis');
